...
def evaluate_filepath(node=None):
    # Fetch the filename as-is (possibly containing expressions)
    selectedNodeFilePathRAW = None
    selectedNodeFilePathEvaluated = None
    if node:
        selectedNodeFilePathRAW = node['file'].getValue()
        selectedNodeFilePathEvaluated = node['file'].getEvaluatedValue()
    else:
    ...
    if len(file_extension)>5:
        file_extension = os.path.splitext(selectedNodeFilePathEvaluated)[1]
    filepath_evaluated = selectedNodeFilePathEvaluated

    print selectedNodeFilePathRAW

    if '%04d' in selectedNodeFilePathRAW or '.value()' in selectedNodeFilePathRAW:
        # If padding was used, we want to make sure that the new path says #### and not e.g. 0140.
        print selectedNodeFilePathEvaluated
    ...
    return filepath_evaluated, file_extension



def checkForFileKnob(node=None):
    try:
        if node:
            selectedNodeFilePath = node['file'].evaluate()
        else:
...
def readFromWrite(nodeName=None):
    ...
    if not error:
        ...
        selectedNodeName = nuke.selectedNode().fullName()
        print selectedNodeName
        if node:
            selectedNodeName = nodeName
        selectedNodeFilePath, file_extension = evaluate_filepath(node)
        #selectedNodeFilePath = evaluate_filepath_stalker( filepath_evaluated=selectedNodeFilePath )
        selectedNodePremult = None
        if node:
            selectedNodePremult = str( node['premultiplied'].getValue() )
        else:
        ...
        # If framerange was found, we need to start getting clever
            ...
            if os.path.exists( nuke.callbacks.filenameFilter(filePath) ):
                ...
                files = [file for file in files if file_extension in file and ".tmp" not in file]
                files = sorted(files)
            else:
